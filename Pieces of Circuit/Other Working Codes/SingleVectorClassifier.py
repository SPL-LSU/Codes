# -*- coding: utf-8 -*-
"""
Created on Thu May 21 12:51:39 2020

Single test vector classification
To use:
    Run code, console asks for input cuz I'm a one-trick pony
    Give a csv file of training data from KNN comparison codes
    Input test vector, numbers separated by spaces
    k=5 in most of our stuff
    tolerance is roughly .9 for teleportation circuit
    
Currently only set up for teleportation circuit. Needs to be generalized, 
but functional for now.

Needs: ability to find ideal (using find_ideal function in ToleranceTest.py)

@author: Margarite LaBorde
"""
...
def main():
    ...
    #number_tests=input("How many vectors are we classifying? ")
    #number_tests=int(number_tests)
    #num=1
    #while num < number_tests
    #num+=1
    arr = input("input numbers in test vector, separated by spaces: ")   # takes the whole line of n numbers
    testvec = list(map(float,arr.split(' ')))
    testvec.append('test')
    testingset.append(testvec)
    ...
    for x in range(len(testingset)): 
        neighbors = getKNeighbors(trainingset, testvec, k,tolerance,ideal) 
