# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 15:53:20 2019
...
def main():
    ...
    qubits=4
    n=2**qubits
    csvpath=["RepeaterTrainingDataQFT.csv","RepeaterTrainingDataHad.csv","RepeaterTrainingDataQFT2.csv"]
    #state=qt.basis(n,0)
    ...
    entanglement_swapping=[qt.cnot(qubits,1,2),hadamaker(qubits,[1,2]),qt.cnot(qubits,1,2),hadamaker(qubits,[1,2]),qt.cnot(qubits,0,1),hadamaker(qubits,[0,1]),qt.cnot(qubits,0,1),hadamaker(qubits,[1,3]),qt.cnot(qubits,1,3),hadamaker(qubits,[1,3]),qt.cnot(qubits,1,2),hadamaker(qubits,[1,2]),qt.cnot(qubits,1,2),hadamaker(qubits,[1,2]),qt.cnot(qubits,1,2)]
    entang=["CNOT","Hadamard","CNOT","Hadamard","CNOT","Hadamard","CNOT","Hadamard","CNOT","Hadamard","CNOT","Hadamard","CNOT","Hadamard","CNOT","Hadamard","CNOT"]
    for j in range(len(entang)):
        entang[j]=entang[j]+",Entanglement"
    circuit=[]
    angles=[]
    for i in range(len(state_creator)):
        circuit.append(state_creator[i])
        circuit.append(state_creator_tags[i])
    for i in range(len(entanglement_swapping)):
        circuit.append(entanglement_swapping[i])
        circuit.append(entang[i])
    #vectors=gen_basis_vectors(n,n,True)
    cat=categorize(circuit)
    #cat2=categorize_broad(circuit)
    alt=[]
    angles=[]
    for i in range(len(circuit)):
        if type(circuit[i]) == str:
    ...
    choice = [4,5,6]
    vector_name = ['QFT','Hadamard 2','Fourier State']
    index = 0
    for x in choice:
        choice = x
        path=csvpath[index]
